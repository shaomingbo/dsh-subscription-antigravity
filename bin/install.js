...
export const PACKAGE_NAME = 'dsh-subscription-antigravity'
// Default source derives from this package's own version so the pinned tag can
// never drift behind a release again (v0.3.0 shipped pinned to v0.2.1).
const PACKAGE_VERSION = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'))
export const DEFAULT_SOURCE = `github:shaomingbo/dsh-subscription-antigravity#v${PACKAGE_VERSION.version}`
...
export function parseArgs(argv) {
  const result = {
    command: 'install',
    profile: 'web',
    source: process.env.DSH_ANTIGRAVITY_SOURCE || DEFAULT_SOURCE,
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--profile') result.profile = argv[++index]
      ...
      if (result.commandExplicit) throw new Error(`unexpected argument: ${arg}`)
      result.command = arg
      result.commandExplicit = true
    }
    else throw new Error(`unknown argument: ${arg}`)
  }
  delete result.commandExplicit
...
Commands:
  install     Add the Antigravity subscription bundle to the profile (default)
  status      Show whether the bundle is installed
  uninstall   Remove the bundle from the profile (idempotent)

Options:
  --profile <name>   Target DSH profile (default: web)
  --source <source>  Package source (default: ${DEFAULT_SOURCE},
                     override with the DSH_ANTIGRAVITY_SOURCE environment variable)
...
It never stops or restarts DSH; restart DSH manually afterwards and
hard-refresh the Web GUI.
OAuth credentials in $DSH_HOME/.antigravity-auth.json are kept on uninstall.`)
}
...
export async function run(argv = process.argv.slice(2), deps = {}) {
  ...
  if (options.command === 'uninstall') {
    ...
    console.log(`OAuth credentials stay in $DSH_HOME/.antigravity-auth.json; delete that file for a full wipe.`)
  ...
  console.log('Restart DSH and hard-refresh the Web page so the bundle enters the boot graph.')
  console.log('Then open Settings → Antigravity and sign in with your Google account.')
}
