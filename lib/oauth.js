...
import { PROVIDER_ID } from './auth-store.js'
...
async function fetchUserEmail(fetchImpl, accessToken) {
  try {
...
export class AntigravityAuth {
  constructor({ store, logger, fetchImpl = fetch, clock = Date.now, discoverProject } = {}) {
    this.store = store
  ...
  configured() {
    return this.store.has(PROVIDER_ID)
  }

  /** Secret-free account snapshot for RPC consumers. */
  status() {
    const credential = this.store.read()
    if (credential === undefined) return { provider: PROVIDER_ID, configured: false }
    return {
  ...
  currentSecrets() {
    const credential = this.store.read()
    return credential ? [credential.access, credential.refresh] : []
  }
  ...
  /**
   * Fresh Google access token; refreshes under a single-flight lock when the
   * stored one is inside the expiry margin. Throws ANTIGRAVITY_AUTH_EXPIRED
   * when Google rejects the refresh token (revoked / re-consent needed).
   */
  async getAccessToken(signal) {
    ...
    if (this.refreshInFlight === undefined) {
      this.refreshInFlight = this.refresh(credential).finally(() => {
        this.refreshInFlight = undefined
      })
    }
  ...
  async refresh(credential) {
    ...
    await this.store.modify(PROVIDER_ID, current => ({
      ...(current ?? credential),
  ...
  async rememberProjectId(projectId) {
    if (typeof projectId !== 'string' || projectId.length === 0) return
    try {
      await this.store.modify(PROVIDER_ID, current => (current === undefined ? undefined : { ...current, projectId }))
    } catch {
      // best effort
    }
  }
  ...
  async runLogin(operation) {
    try {
      ...
      const email = await fetchUserEmail(this.fetchImpl, tokens.access_token)
      let projectId
      try {
      ...
      await this.store.modify(PROVIDER_ID, () => ({
        type: 'oauth',
      ...
      this.setLoginStatus(operation, { kind: 'succeeded', provider: PROVIDER_ID, email })
    } catch (error) {
