...
export function apply(ctx) {
  ...
  void Promise.resolve().then(async () => {
    try {
      ...
      const fallback = createProxy({ auth, accountRouter, client, logger: ctx.logger, port: 0 })
      try {
  ...
  ctx.interval(() => {
    void authReady.then(() => synchronizer.background('timer'))
  }, 10 * 60 * 1000)
    ...
    try {
      ...
      if (endpoint === 'usage') {
        ...
        return success({ usage: await usage.fetchUsage({
          accountId: typeof payloadObject.accountId === 'string' ? payloadObject.accountId : auth.activeAccountId(),
          refresh: payloadObject.refresh === true,
          signal,
        }) })
      }
      if (endpoint === 'usage-all') {
        const payloadObject = requireObject(payload)
        return success({ usages: await usage.fetchAllUsage({ refresh: payloadObject.refresh === true, signal }) })
      }
      if (endpoint === 'models') {
        return success({
