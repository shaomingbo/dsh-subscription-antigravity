 ...
 */
export function createProxy({ auth, client, logger = { warn: () => {}, info: () => {} }, port = 51122, host = '127.0.0.1', emptyRetryDelays = EMPTY_RETRY_DELAYS_MS } = {}) {
  let server
  let actualPort = port
  let starting
  ...
  async function resolveProjectId(token) {
    ...
    if (typeof status.projectId === 'string' && status.projectId.length > 0) return status.projectId
    let discovered
    try {
      discovered = await client.loadCodeAssist(token)
    } catch {
      discovered = undefined
    }
    if (typeof discovered === 'string' && discovered.length > 0) {
      await auth.rememberProjectId(discovered)
      return discovered
    }
    return defaultProjectId(typeof status.email === 'string' ? status.email : 'antigravity-default')
  }
  ...
  async function handleChatCompletions(req, res) {
    ...
    const outcome = await generateWithFallbacks({ token, projectId, params, signal: abort.signal })
    if (!outcome.ok) {
    ...
    while (result.empty && attemptCount < emptyRetryDelays.length && !closed) {
      ...
      attemptCount += 1
      const retry = await generateWithFallbacks({ token, projectId, params, signal: abort.signal })
      if (!retry.ok) break
